...
const FINDING_STATUSES = new Set([
  'NEW','IN_REVIEW','ACCEPTED','REJECTED','INVESTIGATE_LATER',
  'AWAITING_VALIDATION','VALIDATED','SUPERSEDED','DIVERGENCE',
]);
...
findingsRouter.get('/', async (req, res, next) => {
  try {
    ...
    const normalizedAgent = agentItems.map((f) => ({
      ...
      indicator: {
        ...f.indicator,
        currentValue: f.indicator?.values?.[0] || null,
        values: undefined,
      },
    }));
    ...
    console.error('[findings.get]', error);
    next(error);
  }
});
...
findingsRouter.post('/:id/accept', async (req, res, next) => {
  try {
    const finding = await prisma.agentFinding.update({
      ...
      data: { status: 'AWAITING_VALIDATION', reviewedAt: new Date(), reviewedBy: req.body.reviewer || 'Usuário' },
    ...
    await prisma.indicatorHistory.create({
      data: {
        ...
        actor: req.body.reviewer || 'Usuário',
  ...
  try {
    ...
    const finding = await prisma.agentFinding.update({
      ...
      data: {
        ...
        reviewedAt: new Date(),
        reviewedBy: req.body.reviewer || 'Usuário',
      },
    });
    await prisma.indicatorHistory.create({
      data: {
        ...
        actor: req.body.reviewer || 'Usuário',
        details: { findingId: finding.id, reason },
      },
    });
    res.json(finding);
  } catch (error) { next(error); }
...
findingsRouter.post('/:id/validate', async (req, res, next) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!['AWAITING_VALIDATION', 'ACCEPTED', 'NEW', 'IN_REVIEW'].includes(finding.status)) {
        throw new Error('Descoberta não está disponível para validação');
      }
      if (!['NUMERATOR', 'DENOMINATOR', 'FINAL', 'UPDATE'].includes(finding.targetField)) {
        throw new Error('Esta descoberta é apenas uma pista de fonte e não possui campo validável.');
      }
      ...
      const data = {
        ...
        numeratorRaw: current?.numeratorRaw,
        numeratorNumber: current?.numeratorNumber,
        numeratorYear: current?.numeratorYear,
        numeratorSource: current?.numeratorSource,
        numeratorSourceUrl: current?.numeratorSourceUrl,
        denominatorRaw: current?.denominatorRaw,
        denominatorNumber: current?.denominatorNumber,
        denominatorYear: current?.denominatorYear,
        denominatorSource: current?.denominatorSource,
      ...
      if (finding.targetField === 'NUMERATOR') {
        data.numeratorRaw = finding.candidateValueRaw;
        data.numeratorNumber = finding.candidateValueNumber;
        data.numeratorYear = finding.referenceYear;
        data.numeratorSource = finding.sourceName;
        data.numeratorSourceUrl = finding.sourceUrl;
        data.finalRaw = null;
        data.finalNumber = null;
      } else if (finding.targetField === 'DENOMINATOR') {
        data.denominatorRaw = finding.candidateValueRaw;
        data.denominatorNumber = finding.candidateValueNumber;
        data.denominatorYear = finding.referenceYear;
        data.denominatorSource = finding.sourceName;
        data.denominatorSourceUrl = finding.sourceUrl;
        data.finalRaw = null;
        data.finalNumber = null;
      ...
      await tx.agentFinding.update({
        where: { id: finding.id },
        data: { status: 'VALIDATED', reviewedAt: new Date(), reviewedBy: reviewer },
      });
      await tx.indicatorHistory.create({
        data: {
          indicatorId: finding.indicatorId,
          ...
          actor: reviewer,
          details: { findingId: finding.id, newValueId: newValue.id, targetField: finding.targetField },
        },
      });

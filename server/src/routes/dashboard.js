...
dashboardRouter.get('/', async (_req, res, next) => {
  try {
    const [standards, statusGroups, findingsNew, findingsAwaiting, importedAwaiting, lastRun, recentHistory] = await Promise.all([
    ...
    res.json({
      ...
      counts: {
        ...
        acceptedDiscoveries: findingsAwaiting,
        importedAwaitingValidation: importedAwaiting,
      },
      standards: standardStats,
      lastRun,
      attention: attention.map((i) => ({ ...i, currentValue: i.values[0] || null, values: undefined })),
      recentHistory: recentHistory.map((h) => ({
        ...
        indicator: { id: h.indicator.id, code: h.indicator.code, name: h.indicator.name, standard: h.indicator.standard.code },
      })),
    });

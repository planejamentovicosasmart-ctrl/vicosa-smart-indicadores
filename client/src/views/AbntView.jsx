...
import { Search, SlidersHorizontal, LayoutGrid, List, ArrowRight, Database, AlertCircle } from 'lucide-react';
...
export function AbntView({ openIndicator, initialStandard, globalSearch, refreshKey }) {
  ...
  useEffect(()=>{ setError(''); api.indicators({standard,q,status,limit:250}).then(setData).catch(e=>{setData({items:[],total:0});setError(e.message)}); },[standard,q,status,refreshKey]);
  return <div className="page-content">
    ...
    <div className="results-line"><span><strong>{data?.total ?? '—'}</strong> indicadores na ISO {standard}</span><span>Use o filtro "Aguardando validação" para ver dados importados da Geterr/base fornecida.</span></div>
    {mode==='grid' ? <div className="indicator-grid">{data?.items?.map(i=><article className="indicator-card" key={i.id} onClick={()=>openIndicator(i.id)}><div className="indicator-card-top"><span className="code-pill">{i.code}</span><StatusBadge status={i.status}/></div><h3>{i.name}</h3><div className="mini-data"><div><small>Numerador</small><strong>{i.presence?.numerator?'Disponível':'Pendente'}</strong></div><div><small>Denominador</small><strong>{i.presence?.denominator?'Disponível':'Pendente'}</strong></div></div><div className="indicator-card-foot"><span>{i.status==='AWAITING_VALIDATION'?'Dado localizado · aguardando validação':i.findingCount>0?`${i.findingCount} descoberta(s) nova(s)`:'Sem novas descobertas'}</span><ArrowRight size={17}/></div></article>)}</div> : <div className="indicator-table"><div className="indicator-row table-head"><span>Código</span><span>Indicador</span><span>Dados</span><span>Status</span><span></span></div>{data?.items?.map(i=><button className="indicator-row" key={i.id} onClick={()=>openIndicator(i.id)}><span className="code-cell">{i.code}</span><span className="name-cell"><strong>{i.name}</strong><small>ISO {i.standard.code}{i.status==='AWAITING_VALIDATION'?' · dado localizado':i.findingCount?` · ${i.findingCount} descoberta(s)`:''}</small></span><span className="data-cell"><i className={i.presence?.numerator?'ok':''}/><i className={i.presence?.denominator?'ok':''}/><small>{i.presence?.final?'resultado':i.presence?.numerator && i.presence?.denominator?'N + D':'ver detalhes'}</small></span><span><StatusBadge status={i.status}/></span><span><ArrowRight size={16}/></span></button>)}</div>}
